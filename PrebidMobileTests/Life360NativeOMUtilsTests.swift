...
class Life360NativeOMUtilsTests: XCTestCase {
    ...
    // MARK: - Local Prebid Server fixture

    /// Guards the local test setup rather than the SDK: this is the exact `adm` served by
    /// `prebid-server/sample/001_banner/stored_responses/nativo-bid-id-native.json`. If the fixture and the
    /// extractor ever drift, the OM session silently never starts on device, which is slow to diagnose.
    func testLocalPrebidServerNativeFixtureYieldsValidationResource() {
        let adm = #"""
