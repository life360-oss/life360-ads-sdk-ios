...
/// into a 2000-second one, so a stalled Life360 bid request would hang the ad load for ~33 minutes
/// instead of failing fast. These tests pin the conversion on both branches.
class Life360BidRequesterTimeoutTest: XCTestCase {
    ...
    /// Drives a real `Life360BidRequester` against a mock connection and returns the `timeout`
    ...
    private func capturedPostTimeout(file: StaticString = #filePath, line: UInt = #line) throws -> TimeInterval {
        ...
        return try XCTUnwrap(postTimeout, "Life360BidRequester never posted a bid request", file: file, line: line)
    }
}
