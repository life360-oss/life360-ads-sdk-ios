...
/// resolve, or the first Nativo request of a session carries an empty user agent.
///
...
class NativoBidRequesterUserAgentTest: XCTestCase {

    private let knownUserAgent = "Mozilla/5.0 (test) NativoBidRequesterUserAgentTest"
    ...
    private func makeRequester() -> BidRequesterProtocol {
        Factory.createNativoBidRequester(
            ...
            adUnitConfiguration: AdUnitConfig(configId: "config-nativo-ua",
