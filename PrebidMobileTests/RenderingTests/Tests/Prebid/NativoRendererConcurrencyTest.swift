...
/// `PrebidMobilePluginRegister` stores renderers by *name*, so one `NativoRendererInternal` serves
...
class NativoRendererConcurrencyTest: XCTestCase {

    private var renderer: NativoRendererInternal!
    private var interactionStub: StubInteractionDelegate!

    override func setUp() {
        super.setUp()
        renderer = NativoRendererInternal()
    ...
    func testRendererHoldsNoPerLoadState() {
        ...
        XCTAssertTrue(
            ...
            "NativoRendererInternal is a process-wide singleton and must not store per-load "
    ...
    private func makeBannerView(
        ...
        let bid = NativoBidFabricator.makeNativoBid(
            price: 1.0,
            width: width,
            height: height,
            crid: crid
