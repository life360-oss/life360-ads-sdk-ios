...
class Life360AdLayoutTest: XCTestCase {
    ...
    func testExpandChildren_appliesConstraints_whenCreativeSubviewPresent() {
        ...
        let result = Life360AdLayout.expandChildren(displayView, to: banner, withMinimum: 250)
    ...
    func testExpandChildren_matchHeightYieldsToMinimum() {
        ...
        Life360AdLayout.expandChildren(displayView, to: banner, withMinimum: 250)
    ...
    func testExpandFullWidth_replacesExistingWidthConstraint_ratherThanStacking() {
        ...
        Life360AdLayout.expandFullWidth(child)
        Life360AdLayout.expandFullWidth(child)
    ...
    func testExpandFullHeight_replacesExistingHeightConstraint_ratherThanStacking() {
        ...
        Life360AdLayout.expandFullHeight(child)
        Life360AdLayout.expandFullHeight(child)
    ...
    func testExpandFullWidth_isNoOpWithoutSuperview() {
        let orphan = makeView()
        Life360AdLayout.expandFullWidth(orphan)
    ...
    func testWalkFirstChildChain_visitsEveryViewDownToTheStopType() {
        ...
        var visited: [UIView] = []
        Life360AdLayout.walkFirstChildChain(from: a, stopAtType: StopMarkerView.self) { visited.append($0) }
    ...
    func testWalkFirstChildChain_visitsOnlyTheFirstChildBranch() {
        ...
        var visited: [UIView] = []
        Life360AdLayout.walkFirstChildChain(from: root, stopAtType: StopMarkerView.self) { visited.append($0) }

        XCTAssertEqual(visited, [root, firstBranch])
        XCTAssertFalse(visited.contains(secondBranch))
    }

    // MARK: - applyLife360Expansion

    func testApplyLife360Expansion_expandsBannerAndReportsSuccess() {
    ...
    func testApplyLife360Expansion_propagatesMissingCreativeSubview() {
        let container = makeView()
        let banner = makeView(in: container)
        let displayView = makeView(in: banner)
...
/// `Bid.usesLife360Rendering` is the single condition behind two decisions that must agree: whether
/// `Life360RendererInternal` expands the creative, and which delegate callback `BannerView` reports. Derived
/// separately, the two disagree on bids identified only by their loader script.
class BidLife360RenderingTest: XCTestCase {

    func testAdTypeWins_whenPresent() {
        XCTAssertTrue(makeBid(adType: .story, adm: "<html></html>").usesLife360Rendering)
        XCTAssertTrue(makeBid(adType: .ctpVideo, adm: "<html></html>").usesLife360Rendering)
    }

    /// Standard display is the one Life360 type that renders as an ordinary fixed-size banner.
    func testStandardDisplay_doesNotUseLife360Rendering() {
        XCTAssertFalse(makeBid(adType: .standardDisplay, adm: "load.js").usesLife360Rendering)
    }
    ...
    func testLoaderScriptFallback_whenAdTypeIsMissing() {
        XCTAssertTrue(makeBid(adType: nil, adm: "<script src='//x/load.js'></script>").usesLife360Rendering)
        XCTAssertTrue(makeBid(adType: nil, adm: "<script src='//x/LOAD.JS'></script>").usesLife360Rendering)
    }

    func testNoAdTypeAndNoLoaderScript_doesNotUseLife360Rendering() {
        XCTAssertFalse(makeBid(adType: nil, adm: "<html><body>plain</body></html>").usesLife360Rendering)
        XCTAssertFalse(makeBid(adType: nil, adm: nil).usesLife360Rendering)
    }

    // MARK: - Helpers

    private func makeBid(adType: Life360AdType?, adm: String?) -> Bid {
