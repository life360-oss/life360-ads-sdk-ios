...
/// Builds Nativo bid fixtures whose size and creative identity are *distinguishable* per slot.
///
...
enum NativoBidFabricator {
    ...
    static func makeNativoBidResponse(
        price: Double,
        width: Int,
        height: Int,
        crid: String,
    ...
    /// A plain (non-Nativo) `BidResponse`, so a test can mix renderers across concurrent slots.
