...
# Built framework product name for each scheme above. These diverge from the scheme names because the
# targets were renamed to the Life360 Ads SDK while the Lib- scheme names were kept.
products=("Life360AdsSDK" "Life360AdsSDKGAMEventHandlers" "Life360AdsSDKAdMobAdapters" "Life360AdsSDKMAXAdapters")
...
do
	...
	xcodebuild -create-xcframework \
	    -framework "$XCODE_ARCHIVE_DIR/${schemes[$n]}.xcarchive/Products/Library/Frameworks/${products[$n]}.framework" \
	    -debug-symbols "$XCODE_ARCHIVE_DIR_ABSOLUTE/${schemes[$n]}.xcarchive/dSYMs/${products[$n]}.framework.dSYM" \
	    -framework "$XCODE_ARCHIVE_DIR/${schemes[$n]}$POSTFIX_SIMULATOR.xcarchive/Products/Library/Frameworks/${products[$n]}.framework" \
	    -debug-symbols "$XCODE_ARCHIVE_DIR_ABSOLUTE/${schemes[$n]}$POSTFIX_SIMULATOR.xcarchive/dSYMs/${products[$n]}.framework.dSYM" \
	    -output "$OUTPUT_DIR/XC${products[$n]}.xcframework"
	"
