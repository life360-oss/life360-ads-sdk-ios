...
@objcMembers
public class Life360GAMUtils: NSObject {
    ...
    public static func extractClickURL(
        ...
        guard let webView = findWebView(in: bannerView) else {
            Log.debug("Life360GAMClickExtractor: No WKWebView found")
            completion(nil)
            return
        }
        ...
        webView.evaluateJavaScript(extractionJS) { result, error in
            if let error = error {
                Log.debug("Life360GAMClickExtractor: JS error: \(error.localizedDescription)")
                completion(nil)
                return
            }
            if let urlString = result as? String, let url = URL(string: urlString) {
                Log.debug("Life360GAMClickExtractor: Extracted click URL: \(urlString)")
                completion(url)
            } else {
                Log.debug("Life360GAMClickExtractor: No click URL found")
                completion(nil)
            }
        }
    }
    
    public static func fire(url: URL) {
    ...
    private static func findWebView(in bannerView: UIView) -> WKWebView? {
