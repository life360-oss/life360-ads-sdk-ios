 ...
 */
public class Life360Renderer: NSObject, PrebidMobilePluginRenderer, DisplayViewLoadingDelegate {
    ...
    public func displayViewDidLoadAd(_ displayView: UIView) {
        ...
        // Differentiate between Life360 ad rendering or a standard banner ad
        let adm = bid.adm ?? ""
        let isLife360Rendering = adm.range(of: "load.js", options: String.CompareOptions.caseInsensitive) != nil
        
        if isLife360Rendering {
    ...
    private func expandView(_ view: UIView, to parentView: UIView, withMinimum height: CGFloat) {
        ...
        if let pbmWebView = view.subviews.first {
            ...
            let error = NSError(
                domain: "Life360Renderer",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Life360 renderer expected a subview on DisplayView, but none was found."]
            )
            print("\(error)")
        }
    }
